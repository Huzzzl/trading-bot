...
def test_warn_position_exists_without_filled_buy_in_audit() -> None:
    warns = pss.detect_warnings(
        reconciled=[{
            "broker_order_id": "ord-1",
            "client_order_id": "cid-1",
        ...
        open_orders=[],
        position={"symbol": "SPY", "qty": 5.0},
        audit_records=[],
        orphan_claims=[],
    )
    assert any("POSITION_EXISTS_BUT_NO_FILLED_BUY_IN_AUDIT" in w for w in warns)


def test_position_with_filled_buy_is_silent() -> None:
    warns = pss.detect_warnings(
        reconciled=[{
            "broker_order_id": "ord-1",
            "client_order_id": "cid-1",
